    ...
    public class TransactionController : BaseController
    {
        ...
        public IActionResult Index(long id)
        {
            try
            {
                ...
                mscMstPartialViewModel.Placeholder = "Transaction Type";
                ...
                mscMstPartialViewModel.RedirectURL = "/Transaction/Index";
                mscMstPartialViewModel.Type = "TRN";
                if (id > 0)
                {
                    ...
                    {
                        if (DtMisc.Rows[0]["MscActYN"].ToString() == "Yes")
                        {
        ...
        public IActionResult GetReportView(int gridMstId, int pageIndex, int pageSize, string searchValue, string columnName, string sortby)
        {
            ...
            try
            {
                if (gridMstId > 0)
                {
                    ...
                    long userId = GetIntSession("UserId");
                    UserFormRightModel userFormRights = new UserFormRightModel();
                    string currentURL = "/Transaction/Index";
                    userFormRights = GetUserRights(userId, currentURL);
                    if (userFormRights == null)
                    {
                        SetErrorMessage("You do not have right to access requested page. Please contact admin for more detail.");
                    ...
                    getReportDataModel.ControllerName = "Transaction";
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult ExportToExcelPDF(int gridMstId, string searchValue, int type)
        {
            ...
            try
            {
                ...
                if (type == 1)
                {
                    var bytes = Excel(getReportDataModel, "Transaction Type Report", companyDetails.CmpName);
                    return File(
                          bytes,
                ...
                {
                    var bytes = PDF(getReportDataModel, "Transaction Type Report", companyDetails.CmpName);
                    return File(
                          bytes,
                          "application/pdf",
                          "TransactionType.pdf");
            ...
            try
            {
                ...
                if (id > 0)
                {
                    ...
                    if (DtMscMst != null && DtMscMst.Rows.Count > 0)
                    {
                        ...
                        {
                            SetSuccessMessage("Transaction Type Deleted Successfully");
                        }
                    }
                }
            }
