    ...
    public class CountryMasterController : BaseController
    {
        ...
        public IActionResult Index(long id)
        {
            try
            {
                ...
                mscMstPartialViewModel.Placeholder = "Coountry";
                ...
                mscMstPartialViewModel.RedirectURL = "/CountryMaster/Index";
                mscMstPartialViewModel.Type = "COU";
                if (id > 0)
                {
        ...
        public IActionResult GetReportView(int gridMstId, int pageIndex, int pageSize, string searchValue, string columnName, string sortby)
        {
            ...
            try
            {
                if (gridMstId > 0)
                {
                    ...
                    long userId = GetIntSession("UserId");
                    UserFormRightModel userFormRights = new UserFormRightModel();
                    string currentURL = "/CountryMaster/Index";
                    userFormRights = GetUserRights(userId, currentURL);
                    if (userFormRights == null)
                    {
                        SetErrorMessage("You do not have right to access requested page. Please contact admin for more detail.");
                    ...
                    getReportDataModel.ControllerName = "Country";
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult ExportToExcelPDF(int gridMstId, string searchValue, int type)
        {
            ...
            try
            {
                ...
                if (type == 1)
                {
                    var bytes = Excel(getReportDataModel, "Country Report", companyDetails.CmpName);
                    return File(
                          bytes,
                ...
                {
                    var bytes = PDF(getReportDataModel, "Country Report", companyDetails.CmpName);
                    return File(
                          bytes,
                          "application/pdf",
                          "Country.pdf");
