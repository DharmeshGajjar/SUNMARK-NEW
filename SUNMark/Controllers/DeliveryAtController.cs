...
{
    public class DeliveryAtController : BaseController
    {
        ...
        public IActionResult Index(long id)
        {
            try
            {
                ...
                mscMstPartialViewModel.Placeholder = "DeliveryAt";
                ...
                mscMstPartialViewModel.RedirectURL = "/DeliveryAt/Index";
                mscMstPartialViewModel.Type = "DEL";
                if (id > 0)
                {
                    mscMstPartialViewModel.MscVou = Convert.ToInt32(id);
                    SqlParameter[] sqlParameters = new SqlParameter[4];
                    sqlParameters[0] = new SqlParameter("@VOU", id);
                    sqlParameters[1] = new SqlParameter("@Type", "DEL");
        ...
        public IActionResult GetReportView(int gridMstId, int pageIndex, int pageSize, string searchValue, string columnName, string sortby)
        {
            ...
            try
            {
                if (gridMstId > 0)
                {
                    ...
                    long userId = GetIntSession("UserId");
                    UserFormRightModel userFormRights = new UserFormRightModel();
                    string currentURL = "/DeliveryAt/Index";
                    userFormRights = GetUserRights(userId, currentURL);
                    if (userFormRights == null)
                    {
                        SetErrorMessage("You do not have right to access requested page. Please contact admin for more detail.");
                    ...
                    getReportDataModel.ControllerName = "DeliveryAt";
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult ExportToExcelPDF(int gridMstId, string searchValue, int type)
        {
            ...
            try
            {
                ...
                if (type == 1)
                {
                    var bytes = Excel(getReportDataModel, "DeliveryAt Report", companyDetails.CmpName);
                    return File(
                          bytes,
                ...
                {
                    var bytes = PDF(getReportDataModel, "DeliveryAt Report", companyDetails.CmpName);
                    return File(
                          bytes,
                          "application/pdf",
                          "DeliveryAt.pdf");
        ...
        public IActionResult Delete(long id)
        {
            try
            {
                ...
                if (id > 0)
                {
                    ...
                    if (DtMscMst != null && DtMscMst.Rows.Count > 0)
                    {
                        ...
                        {
                            SetSuccessMessage("DeliveryAt Deleted Successfully");
                        }
                    }
                }
            }
            ...
            return RedirectToAction("index", "DeliveryAt");
        }
    }
}
