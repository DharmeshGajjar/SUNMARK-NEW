...
{
    public class SpacificationController : BaseController
    {
        ...
        public IActionResult Index(long id)
        {
            try
            {
                ...
                mscMstPartialViewModel.Placeholder = "Spacification";
                ...
                mscMstPartialViewModel.RedirectURL = "/Spacification/Index";
                mscMstPartialViewModel.Type = "SPE";
                if (id > 0)
                {
        ...
        public IActionResult GetReportView(int gridMstId, int pageIndex, int pageSize, string searchValue, string columnName, string sortby)
        {
            ...
            try
            {
                if (gridMstId > 0)
                {
                    ...
                    long userId = GetIntSession("UserId");
                    UserFormRightModel userFormRights = new UserFormRightModel();
                    string currentURL = "/Spacification/Index";
                    userFormRights = GetUserRights(userId, currentURL);
                    if (userFormRights == null)
                    {
                        SetErrorMessage("You do not have right to access requested page. Please contact admin for more detail.");
                    ...
                    getReportDataModel.ControllerName = "Spacification";
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult ExportToExcelPDF(int gridMstId, string searchValue, int type)
        {
            ...
            try
            {
                ...
                if (type == 1)
                {
                    var bytes = Excel(getReportDataModel, "Spacification Report", companyDetails.CmpName);
                    return File(
                          bytes,
                ...
                {
                    var bytes = PDF(getReportDataModel, "Spacification Report", companyDetails.CmpName);
                    return File(
                          bytes,
                          "application/pdf",
                          "Spacification.pdf");
        ...
        public IActionResult Delete(long id)
        {
            try
            {
                ...
                if (id > 0)
                {
                    ...
                    if (DtMscMst != null && DtMscMst.Rows.Count > 0)
                    {
                        ...
                        {
                            SetSuccessMessage("Spacification Deleted Successfully");
                        }
                    }
                }
            }
            ...
            return RedirectToAction("index", "Spacification");
        }
