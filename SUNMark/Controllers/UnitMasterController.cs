    ...
    public class UnitMasterController : BaseController
    {
        ...
        public IActionResult Index(long id)
        {
            try
            {
                ...
                mscMstPartialViewModel.Placeholder = "Unit";
                ...
                mscMstPartialViewModel.RedirectURL = "/UnitMaster/Index";
                mscMstPartialViewModel.Type = "UNT";
                if (id > 0)
                {
        ...
        public IActionResult GetReportView(int gridMstId, int pageIndex, int pageSize, string searchValue, string columnName, string sortby)
        {
            ...
            try
            {
                if (gridMstId > 0)
                {
                    ...
                    long userId = GetIntSession("UserId");
                    UserFormRightModel userFormRights = new UserFormRightModel();
                    string currentURL = "/UnitMaster/Index";
                    userFormRights = GetUserRights(userId, currentURL);
                    if (userFormRights == null)
                    {
                        SetErrorMessage("You do not have right to access requested page. Please contact admin for more detail.");
                    ...
                    getReportDataModel.ControllerName = "UnitMaster";
                }
            }
            catch (Exception ex)
            {
        ...
        public IActionResult ExportToExcelPDF(int gridMstId, string searchValue, int type)
        {
            ...
            try
            {
                ...
                if (type == 1)
                {
                    var bytes = Excel(getReportDataModel, "Unit Report", companyDetails.CmpName);
                    return File(
                          bytes,
                ...
                {
                    var bytes = PDF(getReportDataModel, "Unit Report", companyDetails.CmpName);
                    return File(
                          bytes,
                          "application/pdf",
                          "Unit.pdf");
